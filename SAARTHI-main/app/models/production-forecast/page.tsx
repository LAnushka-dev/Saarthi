...
export default async function ProductionForecastPage() {
  ...
  const pageHtml = originalHtml.replace(
    "</body>",
    "<script>window.addEventListener('load',function(){if(window.switchPage){window.switchPage('m2')}});</script></body>"
  );

  return (
    <div className="space-y-4">
      <h1 className="text-2xl font-bold text-green-400">Production Forecast Model</h1>
      <p className="text-sm text-slate-300">
        Showing your original integrated frontend (Model 2 tab).
      </p>
      <div className="overflow-hidden rounded-xl border border-green-900/50 bg-black/40">
        <iframe
          title="Original Production Forecast Frontend"
          srcDoc={pageHtml}
